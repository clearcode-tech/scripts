...
4. <b>create-release-branch.sh</b>
   ...
   - Скрипт находит максимальную ветку, содержащую слово `release`, переключается на неё,
     читает MAJOR из текущей версии приложения (`pom.xml` / `build.sbt` / `version.ts`)
     и создаёт новую ветку с именем `{prefix}/{MINOR}` (например `release/52`).
